...
uniform bool hasS5;
uniform SwatchMaterial swatch5;
uniform sampler2D s5Mask;

uniform bool hasS6;
uniform SwatchMaterial swatch6;
uniform sampler2D s6Mask;

uniform bool hasS7;
uniform SwatchMaterial swatch7;
...
vec3 paintRegionBySwatch(vec4 control0Sample, vec4 control1Sample) {
    ...
    outColor = mix(outColor, sampleMaterialSwatch(swatch4, s4Mask), control1Sample.r);
    outColor = mix(outColor, sampleMaterialSwatch(swatch5, s5Mask), control1Sample.g);
    outColor = mix(outColor, sampleMaterialSwatch(swatch6, s6Mask), control1Sample.b);
    
    return outColor;
}

float sampleMetallic(vec4 control0Sample, vec4 control1Sample) {
    ...
    metalnessFactor = mix(metalnessFactor, swatch4.metalness, control1Sample.r);
    metalnessFactor = mix(metalnessFactor, swatch5.metalness, control1Sample.g);
    metalnessFactor = mix(metalnessFactor, swatch6.metalness, control1Sample.g);
    
    return min(max(metalnessFactor, 0.4), 0.8);
}

float sampleRoughness (vec4 control0Sample, vec4 control1Sample) {
    ...
    roughnessFactor = mix(roughnessFactor, swatch4.roughness, control1Sample.r);
    roughnessFactor = mix(roughnessFactor, swatch5.roughness, control1Sample.g);
    roughnessFactor = mix(roughnessFactor, swatch6.roughness, control1Sample.b);

    return roughnessFactor * 0.1;
}

ScratchSwatch sampleScratchColor(vec4 sample0, vec4 sample1) {
    ...
    outSwatch.scratchMetallic = mix(outSwatch.scratchMetallic, swatch4.scratch.scratchMetallic, sample0.r);
    outSwatch.scratchMetallic = mix(outSwatch.scratchMetallic, swatch5.scratch.scratchMetallic, sample0.g);
    outSwatch.scratchMetallic = mix(outSwatch.scratchMetallic, swatch6.scratch.scratchMetallic, sample0.b);
    ...
    outSwatch.scratchBrightness = mix(outSwatch.scratchBrightness, swatch4.scratch.scratchBrightness, sample0.r);
    outSwatch.scratchBrightness = mix(outSwatch.scratchBrightness, swatch5.scratch.scratchBrightness, sample0.g);
    outSwatch.scratchBrightness = mix(outSwatch.scratchBrightness, swatch6.scratch.scratchBrightness, sample0.b);
    ...
    outSwatch.scratchColor = mix(outSwatch.scratchColor, swatch4.scratch.scratchColor, sample0.r);
    outSwatch.scratchColor = mix(outSwatch.scratchColor, swatch5.scratch.scratchColor, sample0.g);
    outSwatch.scratchColor = mix(outSwatch.scratchColor, swatch6.scratch.scratchColor, sample0.b);
    ...
    outSwatch.scratchIOR = mix(outSwatch.scratchIOR, swatch4.scratch.scratchIOR, sample0.r);
    outSwatch.scratchIOR = mix(outSwatch.scratchIOR, swatch5.scratch.scratchIOR, sample0.g);
    outSwatch.scratchIOR = mix(outSwatch.scratchIOR, swatch6.scratch.scratchIOR, sample0.b);
    ...
    outSwatch.scratchRoughness = mix(outSwatch.scratchRoughness, swatch4.scratch.scratchRoughness, sample0.r);
    outSwatch.scratchRoughness = mix(outSwatch.scratchRoughness, swatch5.scratch.scratchRoughness, sample0.g);
    outSwatch.scratchRoughness = mix(outSwatch.scratchRoughness, swatch6.scratch.scratchRoughness, sample0.b);

    return outSwatch;
}

float SampleSwatchEmissive (vec4 sample0, vec4 sample1) {
    ...
    emissiveValue = mix(emissiveValue, swatch4.emissiveIntensity, sample1.r * swatch4.emissiveAmount);
    emissiveValue = mix(emissiveValue, swatch5.emissiveIntensity, sample1.g * swatch5.emissiveAmount);
    emissiveValue = mix(emissiveValue, swatch6.emissiveIntensity, sample1.b * swatch6.emissiveAmount);
...
void main() {
    ...
    // Control Map Samples
    // black in both maps - 0
    // control0 - rgb - 123
    // control1 - rgb - 456
    vec4 sample0 = texture2D(control0, vUv);
    vec4 sample1 = texture2D(control1, vUv);
    vec4 asgSample = texture2D(asg, vUv);    

    // Swatch samples
    vec3 color = paintRegionBySwatch(sample0, sample1);
    float metalnessFactor = sampleMetallic(sample0, sample1);
    float roughnessFactor = sampleRoughness(sample0, sample1);
    
    // Todo: this isn't as "glowy" as I would expect. May need post processing.
    float emissiveStrength = SampleSwatchEmissive(sample0, sample1);
    vec4 emissiveColor = vec4(vec3(1.0) * emissiveStrength, 1.0);

    // Scratches
    ScratchSwatch scratchSample = sampleScratchColor(sample0, sample1);
