...
def convert_bitmap(bitmap_handle, q):
    tex = blender_hi.Texture()
    name = bitmap_handle.replace('\\', '/').split('/')[-1].split('.')[0].split('{')[0]
    tex.readTexture(bitmap_handle, name, None)


def convert_bitmap_Original(bitmap_handle, q):
    tex = Texture()
...
def write_texture(tex, full_save_path):
    if len(DXGI_FORMAT) <tex.texture_format:
        return
...
def write_file(header, tex, full_save_path):
    with open(full_save_path, 'wb') as b:
        for f in fields(header):
            if f.type == np.uint32:
                flipped = "".join(
                    gf.get_flipped_hex(gf.fill_hex_with_zeros(hex(np.uint32(getattr(header, f.name)))[2:], 8), 8))
...
def all_from_directory():
    global folder
    p = [os.path.join(dp, f)[len(directory):].replace("\\", "/") for dp, dn, fn in
         os.walk(os.path.expanduser(directory)) for f in fn if
         f.endswith(".bitmap") and ".chunk" not in f and ".dds" not in f]
    for path in p:
        x = path.split('/')[-1]
...
if __name__ == "__main__":
    ...
    #unpack_directory = "C:/Users/Jorge/Downloads/Compressed/deploy1/HIU"
    unpack_directory = "J:/Games/Halo Infinite Stuf/Extracted/UnPaked/season2/bitmaps"
    ...
    directory = f"{unpack_directory}\\__chore\\pc__\\ui\\"
    directory = f"{unpack_directory}\\__chore\\pc__\\"
    directory = directory.replace("\\", "/")
    out_path = "J:/Games/Halo Infinite Stuf/Extracted/Converted/Textures/DDS"
    all_from_directory()
