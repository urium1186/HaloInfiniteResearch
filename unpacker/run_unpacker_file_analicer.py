...
from unpacker.filtter_to_apply import validString, isInRange, count_file_mapped, count_file_mapped_not, \
    count_file_mapped_error, count_file_mapped_exp, map_tag_names
...
def extract_module(module):
    ...
    for i in range(files_count):
        ...
        bin_temp = fb.read(0x10)
        t1e.hash = bin_temp.hex().upper()               # 0x48 -> 0x58
        t_fb = io.BytesIO(bin_temp)
        t_fb.seek(8)
        t1e.hash_global = gf.read_int32(t_fb)
        t1e.hash_global_hex = t1e.hash[16:24]
    ...
    for i, t1e in enumerate(files):
        ...
        if not validString(t1e.save_path):
            continue
        #print(t1e.save_path)
            ...
            for block in blocks[t1e.first_block_index:t1e.first_block_index+t1e.block_count]:
                if block.b_compressed:
                    ...
                    if len(decomp_save_data) != block.decomp_offset:
                        raise Exception("Skipped data fix")
                    decomp_save_data += decomp
                if not isInRange(len(decomp_save_data), 0, t1e.header_size):
                    debug = True
                    break
        else:
        ...
        if isInRange(len(decomp_save_data), 0, t1e.header_size):
            debug = True
        else:
            continue
...
if __name__ == "__main__":
    ...
    unpack_path = Config.BASE_UNPACKED_PATH # multiplayer
    ...
    deploy_path = Config.DEPLOY_PATH # multiplayer
