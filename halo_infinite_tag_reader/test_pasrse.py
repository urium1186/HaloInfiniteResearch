...
for p in test_path:
    ...
    tag_zone_list[filename.split('\\')[-1]] = (
        f"{tag.ZoneSetInfoHeader.ZoneSetCount}-{tag.ZoneSetInfoHeader.ZoneSetCount1}-" \
        f"{tag.ZoneSetInfoHeader.ZoneSetListOffset}-{tag.ZoneSetInfoHeader.ZoneSetListOffset1}" \
        f" | ZL: {tag.header.ZoneSetSize - 16} DC: {tag.header.DependencyCount} TRC: {tag.header.TagReferenceCount}" \
        f"--- NRxDC: {tag.ZoneSetInfoHeader.ZoneSetListOffset >= tag.header.DependencyCount}" \
        f"--- NRxTRC: {tag.ZoneSetInfoHeader.ZoneSetListOffset <= tag.header.TagReferenceCount}" \
        f"--- NRx0: {tag.ZoneSetInfoHeader.ZoneSetListOffset == 0} : " \
        f"{tag.ZoneSetInfoHeader.ZoneSetListOffset == tag.header.TagReferenceCount == tag.header.DependencyCount == 0}"
        , tag)
...
filename = Config.BASE_UNPACKED_PATH + '__chore\\pc__\\shaders\\default_bitmaps\\bitmaps\\color_red{pc}.bitmap'
